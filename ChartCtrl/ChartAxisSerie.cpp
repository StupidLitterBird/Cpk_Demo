...
CChartAxisSerie::CChartAxisSerie()
...
{
	m_pAxisGrid = new CChartGridSerie();
	m_pAxisLabel = new CChartAxisLabelSerie();
}
...
int CChartAxisSerie::GetPosition()
{
	if (m_bIsHorizontal)
	{
		if (m_bIsSecondary)
...
void CChartAxisSerie::SetParent(CChartCtrl* pParent)
{
...
void CChartAxisSerie::SetHorizontal(bool bHorizontal)
{ 
...
void CChartAxisSerie::Draw(CDC *pDC)
{
	if (!m_bIsVisible)
		return;
...
void CChartAxisSerie::DrawTick(CDC* pDC,
						  double dTickVal)
{
...
bool CChartAxisSerie::IsLabelOnAxis(double TickVal)
{
...
void CChartAxisSerie::DrawLabel(CDC* pDC)
{
	// Draw the axis label.
	CSize LabelSize = m_pAxisLabel->GetSize(pDC);
...
CSize CChartAxisSerie::GetLargestTick(CDC* pDC)
{
	CFont NewFont;
	NewFont.CreatePointFont(m_nFontSize,m_strFontName.c_str(),pDC);
	CFont* pOldFont = pDC->SelectObject(&NewFont);
...
void CChartAxisSerie::SetInverted(bool bInverted)
{
...
void CChartAxisSerie::SetAutomatic(bool bAutomatic)
{
...
void CChartAxisSerie::SetAutomaticMode(EAxisAutoModes AutoMode)
{
	m_AutoMode = AutoMode;
...
void CChartAxisSerie::SetDiscrete(bool bDiscrete)
{
	m_bDiscrete = bDiscrete;
	m_pParentCtrl->RefreshCtrl();
}

void CChartAxisSerie::SetMinMax(double Minimum, double Maximum)
{
...
void CChartAxisSerie::RegisterSeries(CChartSerie* pSeries)
{
...
void CChartAxisSerie::UnregisterSeries(CChartSerie* pSeries)
{
	SeriesList::iterator iter = m_pRelatedSeries.begin();
	for (iter; iter!=m_pRelatedSeries.end(); iter++)
	{
...
bool CChartAxisSerie::RefreshAutoAxis()
{
	RefreshScrollBar();
...
bool CChartAxisSerie::RefreshScreenAutoAxis()
{
	RefreshScrollBar();
	bool bNeedRefresh = false;
	if (m_AutoMode != ScreenAutomatic)
...
void CChartAxisSerie::GetSeriesMinMax(double& Minimum, double& Maximum)
{
	Minimum = 0;
	Maximum = 0;
	double TempMin = 0;
...
void CChartAxisSerie::GetSeriesScreenMinMax(double& Minimum, double& Maximum)
{
	Minimum = 0;
	Maximum = 0;
	double TempMin = 0;
...
long CChartAxisSerie::ValueToScreen(double Value) const
{
    long Offset = 0;
	long retVal = 0;
	if (m_MaxValue==m_MinValue)
...
void CChartAxisSerie::GetScreenMinMax(long& nMin, long& nMax)
{
	double dMin, dMax;
	GetMinMax(dMin, dMax);
	nMin = ValueToScreen(dMin);
	nMax = ValueToScreen(dMax);
}

long CChartAxisSerie::ValueToScreenStandard(double Value) const
{
    long Offset = 0;
	long retVal = 0;
...
double CChartAxisSerie::ScreenToValue(long ScreenVal) const
{
	if (m_MaxValue==m_MinValue)
		return m_MinValue;
...
void CChartAxisSerie::PanAxis(long PanStart, long PanEnd)
{
...
void CChartAxisSerie::SetZoomMinMax(double Minimum, double Maximum)
{
	if (!m_bZoomEnabled)
		return;
...
long CChartAxisSerie::GetAxisLenght() const
{
    long Length = (long)fabs( (m_EndPos-m_StartPos) * 1.0);
    return Length;
}

void CChartAxisSerie::SetVisible(bool bVisible)
{
	m_bIsVisible = bVisible;
	m_pParentCtrl->RefreshCtrl();
}

void CChartAxisSerie::CreateScrollBar()
{
	m_pScrollBar = new CChartScrollBar(this);
	m_pScrollBar->CreateScrollBar(m_pParentCtrl->GetPlottingRect());
}

void CChartAxisSerie::UpdateScrollBarPos()
{
...
void CChartAxisSerie::RefreshScrollBar()
{
	if (m_pScrollBar)
		m_pScrollBar->Refresh();
}

void CChartAxisSerie::SetTextColor(COLORREF NewColor)
{
	m_TextColor = NewColor; 
	m_pParentCtrl->RefreshCtrl();
}

void CChartAxisSerie::SetAxisColor(COLORREF NewColor)
{
	m_AxisColor = NewColor; 
	m_pParentCtrl->RefreshCtrl();
}

void CChartAxisSerie::SetFont(int nPointSize,
...
void CChartAxisSerie::SetMarginSize(bool bAuto, int iNewSize)
{
	m_bAutoMargin = bAuto;
...
void CChartAxisSerie::EnableScrollBar(bool bEnabled)
{
	if (m_pScrollBar)
	{
...
void CChartAxisSerie::SetAutoHideScrollBar(bool bAutoHide)
{
	if (m_pScrollBar)
		m_pScrollBar->SetAutoHide(bAutoHide);
}

bool CChartAxisSerie::GetAutoHideScrollBar() const
{
	if (m_pScrollBar)
...
void CChartAxisSerie::UndoZoom()
{
	SetMinMax(m_UnzoomMin,m_UnzoomMax);
}

void CChartAxisSerie::SetAxisSize(const CRect& ControlRect,
...
int CChartAxisSerie::ClipMargin(CRect ControlRect, CRect& MarginRect, CDC* pDC)
{
	if (!m_bIsVisible)
		return 0;
...
void CChartAxisSerie::Recalculate()
{
	if (m_bAutoTicks)
...
void CChartAxisSerie::GetScrollbarSteps(int& iTotalSteps,
								   int& iCurrentStep)
{
...
void CChartAxisSerie::SetAxisToScrollStep(int iPreviousStep,
									 int iCurrentStep, 
...
BOOL CChartAxisSerie::IsPointInside(const CPoint& screenPoint) const
{
