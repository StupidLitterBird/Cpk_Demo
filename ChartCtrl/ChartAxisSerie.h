...
class CChartCtrl;
class CChartGridSerie;
class CChartSerie;
class CChartAxisLabelSerie;
...
/**
	...
	这个类不能被实例化，但是应该被覆盖以提供所需的功能（这对于标准轴，日期/时间轴和对数轴已经完成）。 
	该类提供了很多功能，但是将定位和标记委托给子类。 默认情况下，类管理连续范围的双精度值（标准轴和日期/时间轴的情况），
	但在某些情况下，这是无效的（例如，对数刻度）。 在这种情况下，你应该覆盖一些特定的功能（例如那些处理滚动条）。 
	有关更多详细信息，请参阅CChartLogarithmicAxis类。
**/
class CChartAxisSerie 
{
	friend CChartCtrl;
	friend CChartGridSerie;
	friend CChartSerie;
...
public:
	...
	**/
	...
	//取得轴名称
	CChartAxisLabelSerie* GetLabel() const	{ return m_pAxisLabel; }
	//! Retrieves the chart axis grid object
	//检索图表轴网格对象
	CChartGridSerie*		 GetGrid()	const	{ return m_pAxisGrid; }
	...
	**/
	//! Powered By :梁颖昌
	void GetScreenMinMax(long& nMin, long& nMax);
...
private:
	...
	CChartGridSerie*			m_pAxisGrid;
	//! The axis label associated with this axis
	CChartAxisLabelSerie*	m_pAxisLabel;
