...
CChartBarSerie::CChartBarSerie(CChartCtrl* pParent, bool bZeroOffset)
 : CChartXYSerie(pParent), m_iBarWidth(0), m_iBorderWidth(1),
   m_BorderColor(RGB(0,0,0)), m_uGroupId(0), m_bGradient(true), 
   m_GradientColor(RGB(255,255,255)), m_GradientType(gtHorizontalDouble),
   m_bHorizontal(false), m_dBaseLine(0), m_bAutoBaseLine(true), m_bStacked(false), m_bZeroOffset(bZeroOffset)
{
...
{ 
	...
	bool bResult = false;
	//ÁºÓ±²ýÐÞ¸Ä		for (unsigned i=uFirst ; i < uLast ; i++)
	for (unsigned i=uFirst ; i < uLast+1 ; i++)
	{
...
bool CChartBarSerie::IsPointOnSerie(int n,const CPoint& screenPoint, unsigned& uIndex) const 
{
	uIndex = INVALID_POINT;
	if (!m_bIsVisible)
		return false;
	...
	{
		CRect BarRect = GetBarRectangle(i,iMinorOffset);
		if (screenPoint.x>=BarRect.left && screenPoint.x<=BarRect.right)
		{
			bResult = true;
			uIndex = i;
			break;
...
int CChartBarSerie::GetMinorOffset() const
{
	...
	for (iter; iter!=m_lstBarSeries.end(); iter++)
	{
		...
		else
		{
			...
			int nBarWidth = (*iter)->GetBarWidth();
