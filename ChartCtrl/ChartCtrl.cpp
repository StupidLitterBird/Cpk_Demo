...
CChartCtrl::~CChartCtrl()
{
	...
	TFormSeriesMap::iterator seriesFormIter = m_mapFormSeries.begin();
	for (seriesFormIter; seriesFormIter != m_mapFormSeries.end(); seriesFormIter++)
	{
		delete (seriesFormIter->second);
	}
...
{
	CChartAxisSerie* pHorizAxis = NULL;
	CChartAxisSerie* pVertAxis = NULL;
...
CChartStandardAxisSerie* CChartCtrl::CreateStandardAxis(EAxisPos axisPos)
{
	CChartStandardAxisSerie* pAxis = new CChartStandardAxisSerie();
	AttachCustomAxis(pAxis, axisPos);
	return pAxis;
}

CChartCustomLabelStandardAxis* CChartCtrl::CreateStandardAxis(EAxisPos axisPos,bool bCustom)
{
	CChartCustomLabelStandardAxis *pAxis=new CChartCustomLabelStandardAxis();
	AttachCustomAxis(pAxis, axisPos);
	return pAxis;
	//
}
...
void CChartCtrl::AttachCustomAxis(CChartCustomLabelStandardAxis *pAxis, EAxisPos axisPos)
{
	...
	if (m_pCAxes[axisPos])
		delete m_pCAxes[axisPos];
	m_pCAxes[axisPos] = pAxis;
}

void CChartCtrl::AttachCustomAxis(CChartAxisSerie* pAxis, EAxisPos axisPos)
{
...
void CChartCtrl::RefreshCtrl()
{
	...
	//绘制图表背景，它不是DrawChart函数的一部分（以避免打印时的背景）。
...
void CChartCtrl::DrawChart(CDC* pDC, CRect ChartRect)
{
	...
	for (n=0;n<4;n++)
	{
		...
		if (m_pCAxes[n])
		{
			m_pCAxes[n]->SetAxisSize(ChartRect,m_PlottingRect);
			m_pCAxes[n]->Recalculate();
			m_pCAxes[n]->ClipMargin(ChartRect,m_PlottingRect,pDC);
		}
	}
	for (n=0;n<4;n++)
	{
	...
	TFormSeriesMap::iterator iterForm = m_mapFormSeries.begin();
	for (iterForm; iterForm != m_mapFormSeries.end(); iterForm++)
	{
		iterForm->second->Draw(pDC);
	}
...
void CChartCtrl::RefreshScreenAutoAxes()
{
	for (int n=0;n<4;n++)
	{
		...
		if (m_pCAxes[n])
			m_pCAxes[n]->RefreshScreenAutoAxis();
	}
}
...
CChartRectangleSubSerie* CChartCtrl::CreateRectangleSerie()
{
	CChartRectangleSubSerie* pNewRectangle = new CChartRectangleSubSerie(this);
	AttachCustomAxis(pNewRectangle);
	return pNewRectangle;
}

void CChartCtrl::AttachCustomAxis(CChartFormSerie* pForm)
{
	if (pForm)
	{
		m_mapFormSeries[pForm->GetSerieId()] = pForm;
...
CChartBarSerie* CChartCtrl::CreateBarSerie(bool bSecondaryHorizAxis, 
										   bool bSecondaryVertAxis,bool bZeroOffset)
{
	CChartBarSerie* pNewSerie = new CChartBarSerie(this, bZeroOffset);
	AttachCustomSerie(pNewSerie, bSecondaryHorizAxis, bSecondaryVertAxis);
	return pNewSerie;
}
								   ...
								   bool bSecondaryVertAxis)
{
	...
	{
		if (m_pCAxes[TopAxis])
			pHorizAxis = m_pCAxes[TopAxis];
		else	
			pHorizAxis = m_pAxes[TopAxis];
	}
	else
	{
		if (m_pCAxes[BottomAxis])
			pHorizAxis = m_pCAxes[BottomAxis];
		else	
			pHorizAxis = m_pAxes[BottomAxis];
	}
	if (bSecondaryVertAxis)
	{
		if (m_pCAxes[RightAxis])
			pVertAxis = m_pCAxes[RightAxis];
		else
			pVertAxis = m_pAxes[RightAxis];
	}
	else
	{
		if (m_pCAxes[LeftAxis])
			pVertAxis = m_pCAxes[LeftAxis];
		else
			pVertAxis = m_pAxes[LeftAxis];
	}
...
void CChartCtrl::RemoveAllSeries()
{
	...
	for (iterForm; iterForm != m_mapFormSeries.end();iterForm++)
	{
		delete iterForm->second;
	}
	m_mapFormSeries.clear();
	RefreshCtrl();
}


CChartAxisSerie* CChartCtrl::GetBottomAxis() const
{
	if (m_pCAxes[BottomAxis])
		return (m_pCAxes[BottomAxis]);
	return (m_pAxes[BottomAxis]);
}

CChartAxisSerie* CChartCtrl::GetLeftAxis() const
{
	if (m_pCAxes[LeftAxis])
		return (m_pCAxes[LeftAxis]);
	return (m_pAxes[LeftAxis]);
}

CChartAxisSerie* CChartCtrl::GetTopAxis() const
{
	if (m_pCAxes[TopAxis])
		return (m_pCAxes[TopAxis]);
	return (m_pAxes[TopAxis]);
}

CChartAxisSerie* CChartCtrl::GetRightAxis() const
{
	if (m_pCAxes[RightAxis])
		return (m_pCAxes[RightAxis]);
...
void CChartCtrl::OnMouseMove(UINT nFlags, CPoint point) 
{
	if (m_bRMouseDown && m_bPanEnabled)
	{
		if (point != m_PanAnchor)
		{
			...
			if (m_pCAxes[LeftAxis])
				m_pCAxes[LeftAxis]->PanAxis(m_PanAnchor.y,point.y);
			if (m_pCAxes[RightAxis])
				m_pCAxes[RightAxis]->PanAxis(m_PanAnchor.y,point.y);
			if (m_pCAxes[BottomAxis])
				m_pCAxes[BottomAxis]->PanAxis(m_PanAnchor.x,point.x);
			if (m_pCAxes[TopAxis])
				m_pCAxes[TopAxis]->PanAxis(m_PanAnchor.x,point.x);
	...
	for (int i=0; i<4; i++)
	{
		...
		if (m_pCAxes[i])
			m_pCAxes[i]->m_pScrollBar->OnMouseLeave();
	}
	...
	if (m_bLMouseDown)
	{
		TFormSeriesMap::iterator itForm = m_mapFormSeries.begin();
		for (itForm; itForm != m_mapFormSeries.end(); itForm++)
		{
//			bool bInRect = itForm->second->PointInRect(point);
			if (itForm->second->GetMove() 
				&& itForm->second->GetClicked() 
				&& itForm->second->PointInRect(point)
				&& !itForm->second->GetRotatiAreaMode())
			{
				CPoint ptMouseOrgin= itForm->second->GetMouseOrgin();
				CPoint ptMoveOrgin = itForm->second->GetMoveOrgin();
				int nThres2X = point.x - ptMouseOrgin.x;
				int nThres2Y = point.y - ptMouseOrgin.y;
				CPoint ptNewMid;
				ptNewMid.x = ptMoveOrgin.x + nThres2X;
				ptNewMid.y = ptMoveOrgin.y + nThres2Y;
				itForm->second->MoveTo(ptNewMid);
			}
			if (itForm->second->GetClicked()						//被点击
				&& itForm->second->GetRotatiAreaMode()//处于旋转区域
				&& itForm->second->GetRotateArea())					//能旋转
			{//旋转角度
				itForm->second->SetAngel(point);
				TRACE("%d,%d\r\n", point.x, point.y);
			}
		}
	}
	...
	if (!m_bLMouseDown)
	{
		...
		for (itForm; itForm != m_mapFormSeries.end(); itForm++)
		{
			CChartFormSerie::enAdjustTheArea enAA = itForm->second->PointInAdjustTheArea(point);
			if (itForm->second->GetClicked() 
				&& itForm->second->CanResize()
				&& enAA != CChartFormSerie::enAdjustTheArea::ADJUST_NONE)
			{
				HCURSOR hCur = LoadCursor(NULL, IDC_SIZEALL);	//IDC_SIZENESW:IDC_SIZENWSE
				::SetCursor(hCur);
			}

			if (itForm->second->GetRotateArea() 
				&& itForm->second->GetClicked()
				&& PtInRect(itForm->second->m_rectRotateArea,point))
			{
				//HCURSOR hCur = LoadCursor(AfxGetApp()->m_hInstance, MAKEINTRESOURCE(IDC_CURSOR1));	//IDC_SIZENESW:IDC_SIZENWSE
				//::SetCursor(hCur);
			}
		}
	}
...
void CChartCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	for (itForm; itForm != m_mapFormSeries.end();itForm++)
	{
		...
		{
			//记录移动前鼠标点击位置
			itForm->second->SetMouseOrgin(point);
			//记录移动前图形的中心位置
			itForm->second->RecordLocMidPt();
			itForm->second->SetResize(bInRect);
			itForm->second->SetClick(bInRect);
		}
		else if (itForm->second->GetRotateArea()&&PtInRect(itForm->second->m_rectRotateArea, point))
		{
			itForm->second->SetRotatiAreaMode();
		}
		else
		{
			itForm->second->SetResize(bInRect);
...
void CChartCtrl::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	if (m_bZoomEnabled)
	{
				  ...
				  (m_rectZoomArea.top!=m_rectZoomArea.bottom))
		{
			...
			if (m_pCAxes[BottomAxis])
			{
				if (m_pCAxes[BottomAxis]->IsInverted())
				{
					MaxVal = m_pCAxes[BottomAxis]->ScreenToValue(m_rectZoomArea.left);
					MinVal = m_pCAxes[BottomAxis]->ScreenToValue(m_rectZoomArea.right);
				}
				else
				{
					MinVal = m_pCAxes[BottomAxis]->ScreenToValue(m_rectZoomArea.left);
					MaxVal = m_pCAxes[BottomAxis]->ScreenToValue(m_rectZoomArea.right);
				}
				m_pCAxes[BottomAxis]->SetZoomMinMax(MinVal,MaxVal);
			}

			if (m_pCAxes[LeftAxis])
			{
				if (m_pCAxes[LeftAxis]->IsInverted())
				{
					MaxVal = m_pCAxes[LeftAxis]->ScreenToValue(m_rectZoomArea.bottom);
					MinVal = m_pCAxes[LeftAxis]->ScreenToValue(m_rectZoomArea.top);
				}
				else
				{
					MinVal = m_pCAxes[LeftAxis]->ScreenToValue(m_rectZoomArea.bottom);
					MaxVal = m_pCAxes[LeftAxis]->ScreenToValue(m_rectZoomArea.top);
				}
				m_pCAxes[LeftAxis]->SetZoomMinMax(MinVal,MaxVal);
			}

			if (m_pCAxes[TopAxis])
			{
				if (m_pCAxes[TopAxis]->IsInverted())
				{
					MaxVal = m_pCAxes[TopAxis]->ScreenToValue(m_rectZoomArea.left);
					MinVal = m_pCAxes[TopAxis]->ScreenToValue(m_rectZoomArea.right);
				}
				else
				{
					MinVal = m_pCAxes[TopAxis]->ScreenToValue(m_rectZoomArea.left);
					MaxVal = m_pCAxes[TopAxis]->ScreenToValue(m_rectZoomArea.right);
				}
				m_pCAxes[TopAxis]->SetZoomMinMax(MinVal,MaxVal);
			}

			if (m_pCAxes[RightAxis])
			{
				if (m_pCAxes[RightAxis]->IsInverted())
				{
					MaxVal = m_pCAxes[RightAxis]->ScreenToValue(m_rectZoomArea.bottom);
					MinVal = m_pCAxes[RightAxis]->ScreenToValue(m_rectZoomArea.top);
				}
				else
				{
					MinVal = m_pCAxes[RightAxis]->ScreenToValue(m_rectZoomArea.bottom);
					MaxVal = m_pCAxes[RightAxis]->ScreenToValue(m_rectZoomArea.top);
				}
				m_pCAxes[RightAxis]->SetZoomMinMax(MinVal,MaxVal);
			}
			RefreshCtrl();
		}
	}
	...
	for (itForm; itForm != m_mapFormSeries.end(); itForm++)
	{
		itForm->second->SetRotatiAreaMode(false);
	}
								...
								const CPoint& screenPoint) const
{
	if (m_pMouseListener)
	{
		...
		for (int i=0; i<4; i++)
		{
			...
			if ( m_pCAxes[i] && m_pCAxes[i]->IsPointInside(screenPoint) )
				m_pMouseListener->OnMouseEventAxis(mouseEvent,screenPoint,m_pCAxes[i]);
		}
	...
	TFormSeriesMap::const_reverse_iterator rIterForm = m_mapFormSeries.rbegin();
	for (rIterForm; rIterForm != m_mapFormSeries.rend(); rIterForm++)
	{
//		if (rIterForm->second->OnMouseEvent(mouseEvent, screenPoint))
//			break;
	}
}
...
void CChartCtrl::SetShowRecttangle(bool bShow)
{
	m_bShowRecttangle = bShow;
}

void CChartCtrl::OnDestroy()
{
	CWnd::OnDestroy();
