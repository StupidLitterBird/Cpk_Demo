...
void CChartPointsSerie::Draw(CDC *pDC)
{
	...
	for (m_uLastDrawnPoint;m_uLastDrawnPoint<(int)GetPointsCount();m_uLastDrawnPoint++)
	{
		...
		case ptVerticalLine:
			long lMin, lMax;
			GetScreenVerticalAxisMinMax(lMin, lMax);
			if (lMin >= 0 || lMax >= 0)
			{
				CPoint ptL(ScreenPoint.x, lMin), ptU(ScreenPoint.x, lMax);

				CPen nPen(PS_DOT, 1, m_colBorder);
				CPen *pOldPenVerLine = pDC->SelectObject(&nPen);
				pDC->MoveTo(ptU);
				pDC->LineTo(ptL);
				pDC->SelectObject(pOldPenVerLine);
				nPen.DeleteObject();
			}
			break;
		}
